...
def init_db():
    with get_db() as conn:
        with conn.cursor() as c:
            ...
            # NOTE: production's real `orders.id` is TEXT (gen_random_uuid()::text),
            # not the SERIAL used in the CREATE TABLE stub above — that stub is a
            # no-op against the live DB since orders already exists there with a
            # different (larger, hand-migrated) schema. notifications.order_id is
            # TEXT to match what's actually live. Confirmed directly against
            # Supabase; do not change orders.id.
            c.execute("""
